...
#
# На сайте характеристика с несколькими значениями работает переключателем вариантов:
# «Фасовка» с кнопками 160 мл / Р9 / С3 у Авроры, «Производитель» с «АО Бакчарское» /
# «Россия» у Бакчарского Великана. Клик вызывает ChangeGoods → /shop/ajax_goods.php,
# и подставляется другой товар со своей ценой.
#
# Наш обход видит только вариант по умолчанию и знает ровно одно значение. Отдай мы его
# в фид — пара значений схлопнется в одно, переключатель исчезнет, а вместе с ним и
# возможность купить остальные фасовки. Поэтому эти поля остаются за 1С навсегда,
# независимо от того, какие настройки импорта включит площадка.
#
# «Доступность» здесь по другой причине: это остаток, он тоже не наш.
VARIANT_AXES = {"фасовка", "производитель", "доступность"}
